...
class IngestionService:
    ...
    async def process(self, job, docs: list[SourceDocument]):
        ...
        for record, document in zip(job.documents,docs):
            try:
                record.stage="extracting"; candidate=await self.provider_factory(document).extract(document)
                record.stage="verifying_citations"
                ...
                for attempt in range(3 if transient else 0):
                    record.retries=attempt+1;job.state="retrying";await asyncio.sleep(2**attempt)
                ...
                else: record.stage="failed";record.error=str(e)
        for r in job.documents:
            if r.stage not in ("failed",): r.stage="resolving_entities"
        grouped,self.last_candidate_pairs=clusters(candidates)
